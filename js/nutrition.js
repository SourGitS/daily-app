...
// nutTab is retired: Nutrition's Today/Foods/Recipes strip became Food's one section strip
// (foodState.tab in js/app.js) plus two supporting overlays. nutSetTab() survives as the
// compatibility shim for callers that still name the old sub-tabs.
...
// Nutrition trends stay in Stats › Nutrition; they did not move into Food › Today, so that
// refresh is unchanged. Only the "am I looking at the day log" test moved, from a top-level
// Nutrition view to Food's Today section.
function nutRefreshConsumers(){nutRefreshLegacyViews();if(typeof renderHome==='function'&&typeof S!=='undefined'&&S.view==='home')renderHome();if(typeof refreshStatsForData==='function')refreshStatsForData(['overview','review','nutrition']);if(typeof foodShowing==='function'&&foodShowing('today'))nutRender();if(typeof foodRefreshSupport==='function')foodRefreshSupport();}
...
// Nutrition's own Today/Foods/Recipes strip is gone: Today is a section of Food's one strip,
// and Foods and Recipes became Food's two supporting screens (Food library, Nutrition Review).
// This stays as the compatibility shim for callers that still name the old sub-tabs — the
// central mapping lives in navResolve()/FOOD_LEGACY_ROUTES in js/app.js, and this defers to it
// rather than carrying a second copy.
function nutSetTab(tab){
  const sub=({today:'today',foods:'library',recipes:'review'})[tab]||'today';
  if(typeof foodGo==='function'){ foodGo(sub); return; }
  nutRender();
}
function nutOpen(meal,highlight){
  nutMeal=meal||'snacks';
  // Always the day log, whichever Food section was last used — this is the "log a meal now"
  // shortcut, not the Food button.
  if(typeof openFoodToday==='function') openFoodToday(); else if(typeof setView==='function') setView('food');
  setTimeout(()=>{const e=highlight&&document.querySelector('[data-nut-entry="'+highlight+'"]');if(e&&typeof safeScrollIntoView==='function')safeScrollIntoView(e,{behavior:'smooth',block:'center'});},80);
}
// The day log. #nutrition-main is unchanged and still lives inside Food › Today, so every
// caller of nutRender() keeps working; only the tab strip it used to draw above itself is gone.
function nutRender(){
  ...
  // Food library is a labelled shortcut FROM Today, not a fifth primary section. Discovery
  // through "+ Add food" (the sheet below) is untouched and is still the main path.
  wrap.innerHTML='<div class="food-today-actions"><button class="nut-small-btn" onclick="foodOpenLibrary()">Food library →</button></div><div id="nut-pane"></div>';
  nutRenderToday(date);
}
...
function nutritionTodayModel(date){
  const summary=nutDaySummary(date),target=nutTarget();
  const previousDate=(()=>{const d=new Date(date+'T12:00:00');d.setDate(d.getDate()-1);return d.toLocaleDateString('en-CA');})();
  return {date,summary,target,progress:target&&summary.calories!=null?Math.min(100,Math.round(summary.calories/target*100)):0,
    macros:summary.macros||{protein:0,carbs:0,fat:0},previousDate,
...
function nutLogSelected(){const f=nutSelected,x=nutSelectionNutrition();if(!f||!x){showToast('Enter a valid quantity');return;}const now=Date.now(),id=nutId('food-'),meal=(document.getElementById('nut-log-meal')||{}).value||nutMeal,recipe=f.kind==='recipe'?f.recipe:null,b=nutSelectedBasis(f),snapName=f.name+(b&&b.optionLabel?' · '+b.optionLabel:'');nutLog.entries[id]={id,date:getLocalDate(),meal,name:snapName,quantity:x.quantity,measureId:x.measure.id,measureLabel:x.measure.label,grams:x.grams,nutrition:x.nutrition,nutritionStatus:x.status,source:f.kind,foodId:f.kind==='catalog'||f.kind==='custom'?f.id:null,recipeId:recipe?recipe.id:null,proteinOptionId:recipe&&b?b.optionId:null,servings:recipe?x.quantity:null,createdAt:now,updatedAt:now,deletedAt:null};nutPrefs.recent=[f.id,...nutPrefs.recent.filter(v=>v!==f.id)].slice(0,20);nutSavePrefs();nutSaveLog();nutCloseFoodSheet();if(typeof foodGo==='function')foodGo('today');nutRefreshConsumers();showToast('Food logged');}
